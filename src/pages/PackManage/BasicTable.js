    ...
    handleOk = (data) => {
        ...
        axios({
            ...
            params:{
                pagesize:10,
                pageindex:1,
                materialCode:encodeURI(data.materialCode)
            }
            ...
            if(this.state.modalType === "add"){
                if(res.data.data instanceof Array && res.data.data.length>0){
                    message.warning("请不要给同一件货品绑定不同包装")
                    return;
                }
                axios({
                ...
                if(data.materialCode !== this.state.dataSource[this.state.editIndex].materialCode && res.data.data instanceof Array && res.data.data.length>0){
                    message.warning("请不要给同一件货品绑定不同包装")
                    return;
                }
                axios({
