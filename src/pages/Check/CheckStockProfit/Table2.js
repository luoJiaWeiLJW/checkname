...
class Table2 extends Component{
    ...
    componentDidMount () {
        ...
        axios({
            url:"/stat/takestock/"+this.props.code+"/type/profit",
            method:"get"
        })
            .then(res =>{
    ...
    showModal = (record) => {
        ...
        axios({
            url:"/stat/takestock/"+this.props.code+"/type/profit/material_code/"+record.material.code+"/goods"
        }).then(res =>{
            if(!checkCode(res.data.code)){
                this.setState({
                    modalData:[]
    ...
    render = ()=> {
        ...
        let  result = (
            <div style={{textAlign:"left"}}>
                ...
                <ModalTable title="盘盈货品列表" visible={this.state.visible} onOk={this.handleOk} onCancel={this.handleCancel} dataSource={this.state.modalData}/>
            </div>
         )
